...
NE_Streams <- st_read(here('Data/Raw/Spatial/cb_2018_us_county_20m.shp'))

#view(NE_Streams)

#Filter for just NE counties

NE_Streams <- NE_Streams %>% 
  filter(STATEFP == 31)
...
ggplot() +
  geom_sf(data = NE_Streams, color = 'black') + 
  geom_sf(data = Gage_Locations_SF, color = 'darkblue', size = 0.8) + theme(
    panel.background = element_rect(color = "white"),
    legend.key = element_rect(color = "white")) +
  labs(title = "Stream Flow Gage Sites, 1928 to 2019")
...
NWIS_Flow_Gage_DF <-
  full_join(Site_Flow_DF, Gage_Locations_DF, by = 'site_no',
...
mapview(NWIS, zcol = "GageHeight", cex = "GageHeight",
        legend = TRUE) +
  mapview(Counties)
...
ggplot() +
  geom_sf(Counties_UTM14) %>%
  aes(NAME == "Lancaster County")

mapview(Counties_UTM14) %>%
  filter(NAME == "Lancaster County")
