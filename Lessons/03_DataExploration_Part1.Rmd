...
> Answer: The console presents a warning about two packages having the same command, such as 'filter.' When using the filter package, the code must specify which package to pull from.
...
#read.csv(file="/home/guest/AU_EDE_Fall2024/Data/Raw/USGS_Site02085000_Flow_Raw.csv",stringsAsFactors = TRUE)
...
#StringsAsFactors: when CVS becomes dataframes, all characters become factors ex. categorical factor

# What happens if we don't assign a name to our imported dataset?
#The console returns an error.
#read.csv(here("./Data/Raw/USGS_Site02085000_Flow_Raw.csv)",stringAsFactors =))
...
> ANSWER: This file is under home/AU_EDE_Fall2024/Data/Metadata. This file describes the collection methodology for this set of data. Future researchers may need these methods to collect similar data sets for comparison. The Read Me file also discusses units and defines terms used in the data set. This is important in ensuring the data is easy to understand for future individuals.
...
# Rename columns
names(USGS.flow.data) <- c("agency_cd", "site_no", "datetime", 
...
> Answer: The blank numeric cells are automatically populated with NA. For characters, blank cells will remain blank.
...
> Answer: For research questions with a singular response, NAs may be useful to retain. For example, answers to a yes or no survey where some participants only responded to some questions (but not all the questions). However, for small data sets where attributes are needed for multiple categories, omitting NAs may be useful. Removing NAs can change the mean and standard deviation. For example, if a power plant has missing data on the annual generation, omitting that plant from the mean annual generation of the data set is necessary.
...
#If there are any missing values, you cannot calculate mean
...
#Adding colclasses overwrites the default on importing columns
...
#Separated by / because this matches the current format of the imported data
