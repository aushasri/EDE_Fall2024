...
NTL.phys.data.TuesdayLake.1990.1999 <- 
  filter(NTL.phys.data, year4 %in% c (1990:1990),
         (lakename == "Tuesday Lake"))

head(NTL.phys.data.TuesdayLake.1990.1999)
...
> Answer: The row numbers of the data could shift around, for example if another title row is added at the top of the table. This could undermine the entire filter system. Filtering by the value in the column ensures that the filters and data desired remain intact even if the row numbers change. We do not have control over the order of row numbers.
...
NTL.phys.data.July.1990.1999 <- 
  NTL.phys.data %>% 
  filter(lakename == "Tuesday Lake") %>% 
  filter(year4 %in% c(1990:1990)) %>% 
  filter(month(sampledate)==7) 
  
  head(NTL.phys.data.July.1990.1999)
