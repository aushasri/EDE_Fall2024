...
#I needed to important data set through the files tab in the bottom right.

NTL.phys.data <- NTL_LTER_Lake_ChemistryPhysics_Raw
...
`&` #If you want two filters to both be true at the same time, use &
`|` #If you want one OR another filter to be true, use |
...
#Filters only impact the rows, not the columns
...
head(NTL.phys.data.TuesdayLake)

```
Question: Why don't we filter using row numbers?

> Answer: The data in a row number can change, which would skew our results. For example, if we reorder the Raw Data into alphabetical order, but we do not change the row numbers in our code, then the data sets in our code will return the wrong rows.
...
> Answer: East Long Lake and Hummingbird Lake have the highest temperatures, from 34 degrees Celsius to 28 degrees Celsius.
...
write.csv(NTL.phys.data.PeterPaul1, row.names = FALSE)

write.csv(NTL.phys.data.PeterPaul1, row.names = FALSE, file = "./Data/Processed/NTL_LTER_Lake_ChemistryPhysics_PeterPaul_Processed.csv")
...
>Answer: This ensures that the data is not accidentally corrupted, changed, or deleted during the analysis process.
It also makes data analysis reproducible and readable.
